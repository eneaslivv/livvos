  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-zinc-500">Upload projects for the website and proposals.</p>
        {isLoading && <div className="text-xs text-zinc-400 mt-2">Loading content...</div>}
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          <input value={portfolioForm.title} onChange={(e) => setPortfolioForm(prev => ({ ...prev, title: e.target.value }))} placeholder="Title" className="px-3 py-2 rounded-lg border border-zinc-200" />
          ...
          <input value={portfolioForm.tags} onChange={(e) => setPortfolioForm(prev => ({ ...prev, tags: e.target.value }))} placeholder="Tags (comma separated)" className="px-3 py-2 rounded-lg border border-zinc-200" />
          <input value={portfolioForm.summary} onChange={(e) => setPortfolioForm(prev => ({ ...prev, summary: e.target.value }))} placeholder="Summary" className="px-3 py-2 rounded-lg border border-zinc-200" />
          <input value={portfolioForm.highlights} onChange={(e) => setPortfolioForm(prev => ({ ...prev, highlights: e.target.value }))} placeholder="Highlights (comma separated)" className="px-3 py-2 rounded-lg border border-zinc-200" />
          <input value={portfolioForm.tech_stack} onChange={(e) => setPortfolioForm(prev => ({ ...prev, tech_stack: e.target.value }))} placeholder="Tech stack (comma separated)" className="px-3 py-2 rounded-lg border border-zinc-200" />
          <input value={portfolioForm.gallery} onChange={(e) => setPortfolioForm(prev => ({ ...prev, gallery: e.target.value }))} placeholder="Gallery URLs (comma separated)" className="px-3 py-2 rounded-lg border border-zinc-200" />
          <input value={portfolioForm.sort_order} onChange={(e) => setPortfolioForm(prev => ({ ...prev, sort_order: e.target.value }))} placeholder="Order" className="px-3 py-2 rounded-lg border border-zinc-200" />
        ...
        <div className="mt-3 flex gap-2">
          <button onClick={savePortfolio} disabled={isSaving} className="px-4 py-2 rounded-lg bg-zinc-900 text-white text-sm">Save</button>
          <button onClick={resetPortfolioForm} className="px-4 py-2 rounded-lg border border-zinc-200 text-sm">New</button>
        </div>
      </div>

      <div>
        <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Blog CMS</h3>
        <p className="text-sm text-zinc-500">Create and publish posts.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
          {posts.map(item => (
            <button
              key={item.id}
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          <input value={postForm.title} onChange={(e) => setPostForm(prev => ({ ...prev, title: e.target.value }))} placeholder="Title" className="px-3 py-2 rounded-lg border border-zinc-200" />
          ...
          <textarea value={postForm.content} onChange={(e) => setPostForm(prev => ({ ...prev, content: e.target.value }))} placeholder="Content" className="px-3 py-2 rounded-lg border border-zinc-200 min-h-[120px]" />
          ...
          <input value={postForm.tags} onChange={(e) => setPostForm(prev => ({ ...prev, tags: e.target.value }))} placeholder="Tags (comma separated)" className="px-3 py-2 rounded-lg border border-zinc-200" />
        ...
        <div className="mt-3 flex gap-2">
          <button onClick={savePost} disabled={isSaving} className="px-4 py-2 rounded-lg bg-zinc-900 text-white text-sm">Save</button>
          <button onClick={resetPostForm} className="px-4 py-2 rounded-lg border border-zinc-200 text-sm">New</button>
        </div>
      </div>
    </div>
  );
