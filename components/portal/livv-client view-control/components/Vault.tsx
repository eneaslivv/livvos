  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5 flex-1">
        {items.map(c => (
          <div key={c.id} className="p-3.5 bg-zinc-50/80 dark:bg-zinc-800/60 border border-zinc-100 dark:border-zinc-800 rounded-xl hover:border-zinc-200 dark:hover:border-zinc-600 transition-all group">
            <div className="flex items-center justify-between mb-2.5">
              <p className="text-[12px] font-semibold text-zinc-600 dark:text-zinc-300">{c.service}</p>
              <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => toggle(c.id)} className="p-1.5 text-zinc-300 dark:text-zinc-600 hover:text-zinc-500 dark:hover:text-zinc-300 rounded-md hover:bg-white dark:hover:bg-zinc-700 transition-all">
                  {showPass[c.id] ? <EyeOff size={13} /> : <Eye size={13} />}
                </button>
                <button onClick={() => copy(c.pass || '', c.id)} className="p-1.5 text-zinc-300 dark:text-zinc-600 hover:text-zinc-500 dark:hover:text-zinc-300 rounded-md hover:bg-white dark:hover:bg-zinc-700 transition-all">
                  {copied === c.id ? <Check size={13} className="text-[#2C0405] dark:text-[#e8a0a2]" /> : <Copy size={13} />}
                </button>
              </div>
            </div>
