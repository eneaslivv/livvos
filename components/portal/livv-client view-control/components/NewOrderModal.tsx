...
const PRIORITIES: { key: NewOrderInput['priority']; label: string; dot: string }[] = [
  ...
  { key: 'urgent', label: 'Urgent', dot: 'bg-rose-500' },
];

/**
 * Client-facing "New request" form. The client submits a work request that
      ...
      >
        {done ? (
          <div className="px-8 py-14 text-center">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-1">Request sent!</h3>
            <p className="text-sm text-zinc-500">The team has received it and will get back to you soon.</p>
          </div>
        ) : (
          <form onSubmit={submit}>
            ...
            <div className="flex items-start justify-between px-7 pt-6 pb-4 border-b border-zinc-100 dark:border-zinc-800">
              <div>
                <div className="text-[10px] font-mono font-medium uppercase tracking-[0.18em] text-[#C4A35A]">New request</div>
                <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mt-1">What do you need?</h3>
                {projectTitle && (
                  <p className="text-[11px] text-zinc-400 mt-0.5">For {projectTitle}</p>
                )}
              </div>
              <button
                type="button"
            ...
            <div className="px-7 py-5 space-y-5">
              ...
              <div>
                <label className="block text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-2">Details <span className="text-zinc-300 normal-case">(optional)</span></label>
                <textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Tell us a bit more: what, by when, any references…"
                  rows={4}
              ...
              <div>
                <label className="block text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-2">Priority</label>
