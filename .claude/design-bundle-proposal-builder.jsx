/* ─── PROPOSAL BUILDER ─────────────────────────────
   Live-previewing deck-style proposal creator.
   Soft steps · multi-tier pricing · editable scope ·
   create-lead-inline · insights.
   ─────────────────────────────────────────────── */
const { useState: useSpb, useMemo: useMpb, useEffect: useEpb, useRef: useRpb } = React;
...
/* ─── Soft step card ─── */
const Step = ({ n, title, sub, meta, touched, children }) => (
...
/* ─── Slide primitive ─── */
const PbSlide = ({ n, total, eyebrow, company, children, cover, wine, fresh }) => (
...
/* ─── Tier subtotal helper ─── */
const tierTotals = (t) => {
  const one = t.lines.filter(l => !l.recur).reduce((s,l) => s + (+l.amount || 0), 0);
  const mo  = t.lines.filter(l =>  l.recur).reduce((s,l) => s + (+l.amount || 0), 0);
...
/* ─── Builder ─── */
const ProposalBuilder = ({ initialLeadId, onClose }) => {
  // Inline-created leads + selection
  const [customLeads, setCustomLeads] = useSpb([]);
  const allLeads = [...customLeads, ...LEADS];
  const [leadId, setLeadId] = useSpb(initialLeadId || 30);
  const lead = allLeads.find(l => l.id === leadId) || allLeads[0];

  const [stageFilter, setStageFilter] = useSpb('all');
  const [icpFilter,   setIcpFilter]   = useSpb('all');
  const [search,      setSearch]      = useSpb('');

  // New-lead inline form
  const [showNewLead, setShowNewLead] = useSpb(false);
  const blankLead = { company: '', contact: '', icp: 'agency', source: 'Direct' };
  const [newLead, setNewLead] = useSpb(blankLead);
  ...
  // Package + editable scope
  const defaultPkg = (icp) =>
    icp === 'agency' ? 'agency' :
    icp === 'product' ? 'product' :
    icp === 'ecom' ? 'ecom' :
    'consult';
  const [pkgId, setPkgId] = useSpb(defaultPkg(lead.icp));
  const pkg = PB_PACKAGES.find(p => p.id === pkgId);

  const [scopeItems, setScopeItems] = useSpb(() => PB_PACKAGES.find(p => p.id === defaultPkg(lead.icp)).scope.slice());

  // Pricing tiers
  const seedTier = (p, name = 'Standard', recommended = true, discount = 0) => ({
    id: 'T' + Date.now() + Math.random().toString(36).slice(2,6),
    name, recommended, discount,
    lines: seedLines(p).map(l => ({ ...l, id: l.id + Math.random().toString(36).slice(2,6) })),
  });
  const [tiers, setTiers] = useSpb([seedTier(PB_PACKAGES.find(p => p.id === defaultPkg(lead.icp)))]);

  // When lead changes, reset package, scope, tiers
  useEpb(() => {
  ...
  // Insights / context add-ons
  const [insights, setInsights] = useSpb([
  ...
  const editInsight = (id, k, v) => setInsights(xs => xs.map(x => x.id === id ? { ...x, [k]: v } : x));
  const delInsight = (id) => setInsights(xs => xs.filter(x => x.id !== id));

  // Brief + AI
  const [brief, setBrief] = useSpb(
  ...
  const [tone, setTone] = useSpb('Direct');
  const [generating, setGenerating] = useSpb(false);
  const [ai, setAi] = useSpb(null);
  const [aiFresh, setAiFresh] = useSpb(false);
  const [aiError, setAiError] = useSpb(null);

  // Timeline
  const [startDate, setStartDate] = useSpb('Jun 02');
  const proposalDate = 'May 19, 2026';

  // Custom info
  const [links, setLinks] = useSpb([
    { id: 1, label: 'Sunnyside case study', url: 'livvvv.com/work/sunnyside' },
  ]);
  const [cases, setCases]   = useSpb(['sunnyside', 'aire']);
  const [notes, setNotes]   = useSpb('');
  ...
  // Line + scope + tier helpers
  const editScope = (idx, v) => setScopeItems(xs => xs.map((s,i) => i === idx ? v : s));
  const delScope  = (idx)    => setScopeItems(xs => xs.filter((_,i) => i !== idx));
  const addScope  = ()       => setScopeItems(xs => [...xs, '']);

  const addTier = (tplName = 'Custom') => setTiers(ts => [...ts, seedTier(pkg, tplName, false, 0)]);
  const delTier = (id) => setTiers(ts => ts.length > 1 ? ts.filter(t => t.id !== id) : ts);
  const editTier = (id, k, v) => setTiers(ts => ts.map(t => t.id === id ? { ...t, [k]: v } : t));
  const setRecommended = (id) => setTiers(ts => ts.map(t => ({ ...t, recommended: t.id === id ? !t.recommended : false })));

  const addLineTo = (tierId) => editTier(tierId, 'lines', [
    ...(tiers.find(t => t.id === tierId).lines),
    { id: 'L' + Date.now() + Math.random().toString(36).slice(2,5), label: '', amount: 0, recur: false }
  ]);
  const editLineIn = (tierId, lineId, k, v) => {
    const t = tiers.find(t => t.id === tierId);
    editTier(tierId, 'lines', t.lines.map(l => l.id === lineId ? { ...l, [k]: v } : l));
  };
  const delLineIn = (tierId, lineId) => {
    const t = tiers.find(t => t.id === tierId);
    editTier(tierId, 'lines', t.lines.filter(l => l.id !== lineId));
  };

  const addLink = () => setLinks(ls => [...ls, { id: Date.now(), label: '', url: '' }]);
  const editLink = (id, k, v) => setLinks(ls => ls.map(l => l.id === id ? { ...l, [k]: v } : l));
  const delLink  = (id) => setLinks(ls => ls.filter(l => l.id !== id));
  const toggleCase = (id) => setCases(cs => cs.includes(id) ? cs.filter(c => c !== id) : [...cs, id]);
...
Return ONLY valid JSON (no fences, no preamble). Shape:
{
  ...
  const N = (key) => String(slides.indexOf(key) + 1).padStart(2, '0');
    ...
    <div className="pb-root" data-screen-label="Proposals · Builder">
      <div className="pb-head">
        ...
        <div className="pb-head-actions">
          <button className="dx-btn"><OS_ICON name="docs" size={12}/><span>Save draft</span></button>
      ...
      <div className="pb-grid">
        ...
        <div className="pb-form">
          ...
          {/* 01 — LEAD (with create-new-inline) */}
            ...
            <div className="pb-filters">
              ...
              <div className="pb-chips">
                {[['all','All stages'],['contacted','Contacted'],['call','Call'],['call-done','Call done'],['proposal','Proposal'],['won','Won']].map(([id, lbl]) => (
                  <button key={id} className={`pb-chip ${stageFilter === id ? 'on' : ''}`} onClick={() => setStageFilter(id)}>{lbl}</button>
                ))}
              </div>
              <div className="pb-chips">
                {[['all','All ICPs'],['agency','Agency'],['consult','Consultant'],['product','SaaS'],['ecom','E-com']].map(([id, lbl]) => (
                  <button key={id} className={`pb-chip ${icpFilter === id ? 'on' : ''}`} onClick={() => setIcpFilter(id)}>{lbl}</button>
                ))}
              </div>
            </div>
          ...
          <Step n="03" title="What you want to do" sub="A few sentences in your words. AI rewrites it in Livv voice." touched={t_brief}>
            <textarea
              ...
              placeholder="Describe what the client needs. 2–4 sentences is enough."
            />
          ...
          <Step n="04" title="Package & scope" sub="Start from a package, then edit the deliverables freely." meta={`${pkg.weeks} weeks · ${scopeItems.filter(s=>s.trim()).length} items`} touched>
            <div className="pb-pkgs">
              {PB_PACKAGES.map(p => (
                <button key={p.id} className={`pb-pkg ${pkgId === p.id ? 'on' : ''}`} onClick={() => { setPkgId(p.id); setScopeItems(p.scope.slice()); setTiers([seedTier(p)]); }} style={{ '--c': p.color }}>
          ...
          <Step
            n="05"
            title="Pricing tiers"
            sub="Send one offer or compare a few. Toggle the chip to recommend a tier."
                        ...
                        {tier.lines.map(l => (
                          <div key={l.id} className="pb-line">
                            <input
                              ...
                              onChange={e => editLineIn(tier.id, l.id, 'label', e.target.value)}
                            />
                            <input
                              className="amount"
                              type="number"
                              value={l.amount}
                              placeholder="0"
                              onChange={e => editLineIn(tier.id, l.id, 'amount', e.target.value === '' ? '' : +e.target.value)}
                            />
                            <button
                              className={`pb-line-recur ${l.recur ? 'on' : ''}`}
                              onClick={() => editLineIn(tier.id, l.id, 'recur', !l.recur)}
                            >
                              {l.recur ? '/ mo' : 'one-time'}
                            </button>
          ...
          <Step
            n="06"
            title="Custom info"
            sub="Selected work, custom links, a note for the client. All optional."
            meta={`${validLinks.length} links · ${cases.length} cases`}
            touched={t_custom}
          >
            <div className="pb-field">
              ...
              <div className="pb-cases">
                {PB_PORTFOLIO.map(c => (
                  <button key={c.id} className={`pb-case ${cases.includes(c.id) ? 'on' : ''}`} style={{ '--c': c.color }} onClick={() => toggleCase(c.id)}>
                ...
                {links.map(l => (
                  <div key={l.id} className="pb-link">
                    ...
                    <input
                      ...
                      value={l.label}
                      placeholder="Label"
                      onChange={e => editLink(l.id, 'label', e.target.value)}
                    />
                    <input
                      className="url"
                      type="text"
                      value={l.url}
                      placeholder="https://…"
                      onChange={e => editLink(l.id, 'url', e.target.value)}
                    />
            ...
            <div className="pb-field" style={{ marginTop: 16 }}>
              ...
              <textarea
                ...
                placeholder="Anything personal — a thank-you, conditions, fine print."
              />
            </div>
          </Step>
            ...
            {selectedCases.length > 0 && (
              <PbSlide n={N('work')} total={total} eyebrow="Selected work" company={lead.company}>
                ...
                <div className="pb-work-grid">
                  {selectedCases.map(c => (
                    <div key={c.id} className="pb-work-card" style={{ '--c': c.color }}>
...
Object.assign(window, { ProposalBuilder, PB_PACKAGES, PB_INDUSTRY, PB_ICP_COLOR, PB_PORTFOLIO });
