...
('Lucas Martinez', 'lucas@startup.io', 'StartupXYZ', '+1-555-0456', 'prospect', 'CTO técnico, enfocado en escalabilidad y performance.', 'SaaS', '456 Innovation Ave, Austin, TX'),
('Sarah Jenkins', 'sarah@boutique.co', 'Fashion Boutique', '+1-555-0789', 'active', 'Dueña de boutique de moda. Necesita e-commerce moderno.', 'Fashion', '789 Style Blvd, New York, NY');

INSERT INTO client_messages (client_id, sender_type, sender_name, message, created_at) VALUES 
((SELECT id FROM clients WHERE email = 'sofia@techcorp.com'), 'client', 'Sofia Rodriguez', 'Hola! Estoy muy interesada en un rebranding completo para TechCorp. ¿Podemos agendar una llamada?', NOW() - INTERVAL '2 days'),
((SELECT id FROM clients WHERE email = 'sofia@techcorp.com'), 'user', 'Eneas', '¡Hola Sofia! Claro, me encantaría discutir tu proyecto. ¿Qué día te vendría bien?', NOW() - INTERVAL '1 day'),
((SELECT id FROM clients WHERE email = 'lucas@startup.io'), 'client', 'Lucas Martinez', 'Necesito ayuda con la arquitectura de nuestra API. Estamos creciendo rápidamente.', NOW() - INTERVAL '3 days');
...
((SELECT id FROM clients WHERE email = 'lucas@startup.io'), 'Revisar documentación de API', 'Analizar la arquitectura actual y proponer mejoras', 'medium', NOW() + INTERVAL '5 days'),
((SELECT id FROM clients WHERE email = 'sarah@boutique.co'), 'Diseñar mockups de e-commerce', 'Crear 3 propuestas de diseño para la tienda online', 'high', NOW() + INTERVAL '2 days');
...
((SELECT id FROM clients WHERE email = 'lucas@startup.io'), 'Eneas', 'meeting', 'Reunión Zoom de 30 minutos - revisaron requisitos técnicos'),
((SELECT id FROM clients WHERE email = 'sarah@boutique.co'), 'Eneas', 'email', 'Envió portfolio de diseño y casos de éxito en e-commerce');