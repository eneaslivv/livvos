...
-- 6. POLICIES FOR TASKS (Refers to calendar_tasks)

-- View Policy:
-- Inherits project visibility. If you can see the project, you can see its tasks.
DROP POLICY IF EXISTS "View Tasks Policy" ON calendar_tasks;
CREATE POLICY "View Tasks Policy" ON calendar_tasks
FOR SELECT
USING (
  EXISTS (
...
DROP POLICY IF EXISTS "View Tasks Optimized" ON calendar_tasks;
CREATE POLICY "View Tasks Optimized" ON calendar_tasks
FOR SELECT
USING (
  has_permission('projects', 'view_all')
  OR
...
DROP POLICY IF EXISTS "View Leads Policy" ON leads;
